            ...
            <div className="mt-3 space-y-2 border-t border-amber-700/20 pt-3">
              ...
              {AVATAR_FRAMES.filter(f => ownedFrames.includes(f.id) && !(f as any).premiumOnly && !(f as any).seasonOnly).map(frame => (
                ...
                >
                  ...
                  <span className="text-amber-100 text-sm">{locale === 'kk' ? (frame as any).nameKk : locale === 'en' ? (frame as any).nameEn || frame.name : locale === 'uk' ? ((frame as any).nameUk || frame.name) : locale === 'ka' ? ((frame as any).nameKa || frame.name) : locale === 'az' ? ((frame as any).nameAz || frame.name) : locale === 'uz' ? ((frame as any).nameUz || frame.name) : frame.name}</span>
              ...
              {AVATAR_FRAMES.filter(f => (f as any).premiumOnly).map(frame => (
                isPremium ? (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-1.5">
                        <span className="text-amber-200/50 text-sm">{locale === 'kk' ? (frame as any).nameKk : locale === 'en' ? (frame as any).nameEn || frame.name : locale === 'uk' ? ((frame as any).nameUk || frame.name) : locale === 'ka' ? ((frame as any).nameKa || frame.name) : locale === 'az' ? ((frame as any).nameAz || frame.name) : locale === 'uz' ? ((frame as any).nameUz || frame.name) : frame.name}</span>
                        <span className="text-[9px] font-bold px-1 py-0.5 rounded bg-yellow-500/20 text-yellow-400 border border-yellow-500/30">PREMIUM</span>
                      </div>
