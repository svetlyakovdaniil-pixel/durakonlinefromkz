...
export default function ProfileDrawer({
  ...
  const panel = open ? (
    ...
    >
      {/* Header */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', borderBottom: '1px solid rgba(180,130,30,0.2)', flexShrink: 0 }}>
        <span style={{ color: '#fde68a', fontWeight: 600, fontSize: '16px' }}>{t('profile.title')}</span>
        <button
          onClick={() => setOpen(false)}
          style={{ width: 32, height: 32, borderRadius: '50%', background: 'rgba(0,0,0,0.4)', border: 'none', color: '#fde68a', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
        >
          <X style={{ width: 16, height: 16 }} />
        </button>
      </div>
      {/* Tabs */}
      <Tabs defaultValue={initialTab === 'history' ? 'history' : 'profile'} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
        <TabsList className="mx-4 mt-2 bg-[#1a2d45] border border-amber-700/20 w-auto" style={{ flexShrink: 0 }}>
      ...
      <div style={{ flexShrink: 0, padding: '12px 16px', borderTop: '1px solid rgba(180,130,30,0.2)' }}>
        <button
          onClick={() => setOpen(false)}
          style={{ width: '100%', padding: '12px', borderRadius: 12, background: 'rgba(251,191,36,0.12)', border: '1px solid rgba(251,191,36,0.3)', color: '#fbbf24', fontWeight: 600, fontSize: 14, cursor: 'pointer' }}
        >
            ...
            <div className="mt-3 space-y-2 border-t border-amber-700/20 pt-3">
              ...
              {AVATAR_FRAMES.filter(f => ownedFrames.includes(f.id) && !(f as any).premiumOnly && !(f as any).seasonOnly).map(frame => (
                ...
                >
                  ...
                  <span className="text-amber-100 text-sm">{locale === 'kk' ? (frame as any).nameKk : locale === 'en' ? (frame as any).nameEn || frame.name : locale === 'uk' ? ((frame as any).nameUk || frame.name) : locale === 'ka' ? ((frame as any).nameKa || frame.name) : locale === 'az' ? ((frame as any).nameAz || frame.name) : locale === 'uz' ? ((frame as any).nameUz || frame.name) : locale === 'pl' ? ((frame as any).namePl || frame.name) : frame.name}</span>
              ...
              {AVATAR_FRAMES.filter(f => (f as any).premiumOnly).map(frame => (
                isPremium ? (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-1.5">
                        <span className="text-yellow-300 text-sm font-semibold">{locale === 'kk' ? (frame as any).nameKk : locale === 'en' ? (frame as any).nameEn || frame.name : locale === 'uk' ? ((frame as any).nameUk || frame.name) : locale === 'ka' ? ((frame as any).nameKa || frame.name) : locale === 'az' ? ((frame as any).nameAz || frame.name) : locale === 'uz' ? ((frame as any).nameUz || frame.name) : locale === 'pl' ? ((frame as any).namePl || frame.name) : frame.name}</span>
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-1.5">
                        <span className="text-amber-200/50 text-sm">{locale === 'kk' ? (frame as any).nameKk : locale === 'en' ? (frame as any).nameEn || frame.name : locale === 'uk' ? ((frame as any).nameUk || frame.name) : locale === 'ka' ? ((frame as any).nameKa || frame.name) : locale === 'az' ? ((frame as any).nameAz || frame.name) : locale === 'uz' ? ((frame as any).nameUz || frame.name) : locale === 'pl' ? ((frame as any).namePl || frame.name) : frame.name}</span>
                        <span className="text-[9px] font-bold px-1 py-0.5 rounded bg-yellow-500/20 text-yellow-400 border border-yellow-500/30">PREMIUM</span>
                      </div>
