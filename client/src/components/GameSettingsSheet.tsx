...
export default function GameSettingsSheet({ onLeaveGame, children, roomPenalty = 0, isTutorial = false }: GameSettingsSheetProps) {
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent side="right" className="bg-[#0f2035] border-l border-amber-700/30 text-amber-100 w-[300px] sm:w-[380px] max-h-[100dvh] overflow-y-auto" style={{ paddingTop: 'calc(env(safe-area-inset-top) + 16px)', paddingBottom: 'calc(env(safe-area-inset-bottom) + 16px)' }}>
            ...
            <AlertDialogContent className="bg-[#1a2d45] border-amber-700/30 text-amber-100 max-w-[calc(100vw-2rem)] sm:max-w-md">
              <AlertDialogHeader>
                <AlertDialogTitle className="text-amber-100">
                  {t('game.leaveConfirmTitle')}
                </AlertDialogTitle>
                <AlertDialogDescription className="text-amber-200/60 space-y-2">
                  <span className="block">
                    {isTutorial ? t('game.leaveTutorialDesc') : t('game.leaveConfirmDesc').replace('{penalty}', String(roomPenalty))}
