                  ...
                  <div key={tableId} className="bg-[#0f2035]/80 border border-amber-700/20 rounded-xl p-4">
                    <div className="flex flex-col gap-3">
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          <h3 className="text-amber-100 font-bold text-sm">{locale === 'kk' ? table.nameKk : locale === 'en' ? table.nameEn : locale === 'uk' ? (table.nameUk || table.name) : locale === 'ka' ? ((table as any).nameKa || table.name) : locale === 'az' ? ((table as any).nameAz || table.name) : locale === 'uz' ? ((table as any).nameUz || table.name) : table.name}</h3>
                        </div>
                        {isOwned ? (
                          <div className="flex items-center gap-1.5 text-green-400 text-sm font-medium">
                            <Check className="w-4 h-4" /><span>{t('shop.purchased')}</span>
                          ...
                          <div className="flex items-center gap-3">
                            <Button className="bg-amber-600 hover:bg-amber-500 text-white text-sm h-9 px-4"
                              onClick={() => setConfirmPurchase({ type: 'table', id: tableId, name: locale === 'kk' ? table.nameKk : locale === 'en' ? table.nameEn : locale === 'uk' ? (table.nameUk || table.name) : locale === 'ka' ? ((table as any).nameKa || table.name) : locale === 'az' ? ((table as any).nameAz || table.name) : locale === 'uz' ? ((table as any).nameUz || table.name) : table.name, price: effectivePrice })}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <h3 className="text-amber-100 font-bold text-sm">
                            {locale === 'kk' ? frame.nameKk : locale === 'en' ? (frame as any).nameEn || frame.name : locale === 'uk' ? ((frame as any).nameUk || frame.name) : locale === 'ka' ? ((frame as any).nameKa || frame.name) : locale === 'az' ? ((frame as any).nameAz || frame.name) : locale === 'uz' ? ((frame as any).nameUz || frame.name) : frame.name}
                          </h3>
                          {isPremiumFrame && (
                        ...
                        <p className="text-amber-200/50 text-xs mb-3">
                          {locale === 'kk' ? frame.descriptionKk : locale === 'en' ? (frame as any).descriptionEn || frame.description : locale === 'uk' ? ((frame as any).descriptionUk || frame.description) : locale === 'ka' ? ((frame as any).descriptionKa || frame.description) : locale === 'az' ? ((frame as any).descriptionAz || frame.description) : locale === 'uz' ? ((frame as any).descriptionUz || frame.description) : frame.description}
                        </p>
                        {isSeasonFrame ? (
                          isOwned ? (
                          ...
                          <div className="flex items-center gap-3">
                            <Button className="bg-amber-600 hover:bg-amber-500 text-white text-sm h-9 px-4"
                              onClick={() => setConfirmPurchase({
                                ...
                                name: locale === 'kk' ? frame.nameKk : locale === 'en' ? (frame as any).nameEn || frame.name : locale === 'uk' ? ((frame as any).nameUk || frame.name) : locale === 'ka' ? ((frame as any).nameKa || frame.name) : locale === 'az' ? ((frame as any).nameAz || frame.name) : locale === 'uz' ? ((frame as any).nameUz || frame.name) : frame.name,
