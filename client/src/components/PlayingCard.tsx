...
function NumberCard({ card }: { card: Card }) {
  ...
  return (
    <div className="w-full h-full bg-white rounded-lg flex flex-col items-center justify-between p-1 relative overflow-hidden">
      {/* Ornament background */}
      ...
      {/* Center suit */}
      <div className="z-10 text-2xl" style={{ color }}>
        {symbol}
      </div>
      {/* Bottom-right rank */}
...
export default function PlayingCard({ card, faceDown, selected, playable, small, onClick, className }: PlayingCardProps) {
  const sizeClasses = small ? 'w-12 h-18' : 'w-16 h-24 sm:w-20 sm:h-30';
